            ...
            <div class="row">
              <div class="col-lg-12 fv-row">
                <div class="position-relative d-flex align-items-center">
                  ...
                  <input type="date" name="date" id="date" class="form-control form-control-solid ps-12" placeholder="Select a date" name="date" />
                </div>
              </div>
            </div>
          </div>
    ...
    $('#date').change(function() {
      ...
      $.ajax({
        ...
        success: function(response)
        {
          ...
          {
            var slot = response[i];
            var button = $('<button>').addClass('btn btn-primary slot-btn').text(slot.start_time + ' - ' + slot.finish_time).val(slot.id);
            if (slot.is_booked) {
