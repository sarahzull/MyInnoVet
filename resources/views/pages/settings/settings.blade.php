
@extends('layout.master')
@section('title', 'Settings')
@section('breadcrumb', 'Settings')

@section('content')

<div class="card card-flush pb-0 bgi-position-y-center bgi-no-repeat mb-10">
  <div class="card-header pt-10">
    <div class="d-flex align-items-center">
      <div class="d-flex flex-column">
        <h2 class="mb-1">Settings List</h2>
      </div>
    </div>
  </div>
  <div class="card-body pb-0">
    <div class="table-responsive">
      <table id="kt_datatable" class="table table-bordered gy-3 gs-3 align-middle">
        <thead>
          <tr class="text-start text-gray-700 fw-bold fs-7 text-uppercase bg-light">
            <th class="w-10px pe-2">No</th>
            <th class="min-w-250px">Name</th>
            <th class="min-w-125px">Last Modified</th>
          </tr>
        </thead>
        <tbody>
            <tr>
              <td>1</td>
              <th scope="row" class="text-center">
                <div class="d-flex align-items-center">
                  <span class="svg-icon svg-icon-muted svg-icon-2x"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path opacity="0.3" d="M10 4H21C21.6 4 22 4.4 22 5V7H10V4Z" fill="currentColor"/>
                    <path d="M9.2 3H3C2.4 3 2 3.4 2 4V19C2 19.6 2.4 20 3 20H21C21.6 20 22 19.6 22 19V7C22 6.4 21.6 6 21 6H12L10.4 3.60001C10.2 3.20001 9.7 3 9.2 3Z" fill="currentColor"/>
                    </svg>
                  </span>                             
                  <a href="{{ route('species.index') }}" class="text-gray-800 text-hover-primary px-2 fs-6">Species</a>
                </div>
              </th>
              <td>{{ $species->created_at ?? 'N/A' }}</td>
            </tr>
            <tr>
              <td>2</td>
              <th scope="row" class="text-center">
                <div class="d-flex align-items-center">
                  <span class="svg-icon svg-icon-muted svg-icon-2x"><svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path opacity="0.3" d="M10 4H21C21.6 4 22 4.4 22 5V7H10V4Z" fill="currentColor"/>
                    <path d="M9.2 3H3C2.4 3 2 3.4 2 4V19C2 19.6 2.4 20 3 20H21C21.6 20 22 19.6 22 19V7C22 6.4 21.6 6 21 6H12L10.4 3.60001C10.2 3.20001 9.7 3 9.2 3Z" fill="currentColor"/>
                    </svg>
                  </span>                             
                  <a href="{{ route('species.index') }}" class="text-gray-800 text-hover-primary px-2 fs-6">Appointment Type</a>
                </div>
              </th>
              <td>{{ $species->created_at ?? 'N/A' }}</td>
            </tr>
        </tbody>
      </table>
    </div>
  </div>
</div>

@endsection
